...
# ==============================================================================
Cpa_BASE      = 4.18       # J/g C
Tain_BASE     = 20.0       # C - constante interna, no ajustable
Teb_BASE      = 90.0       # C
lambda_a_BASE = 2283.0     # J/g a 90C

VMV_BASE      = 1500.0     # cm3
C0_BASE       = 0.01103    # g/cm3
Deff_BASE     = 4.8336e-4  # 1/s
rho_AE_BASE   = 0.88       # g/mL
...
# 2. FUNCIONES
# ==============================================================================
def calcular_mv(PeR, eta_c):
    den = Cpa_BASE*(Teb_BASE - Tain_BASE) + lambda_a_BASE
    return (1 - eta_c)*PeR/den if den > 0 else 0.0

def mAE_acum(tf_s, F1, F2, n_terms=10):
    total = 0.0
    for n in range(n_terms):
        term = (2*n+1)**2
        if F2*term > 0:
            total += (1 - np.exp(-F2*term*tf_s)) / (F2*term)
    return F1*total

def mAE_inst(t_s, F1, F2, n_terms=10):
    return F1 * sum(np.exp(-F2*(2*n+1)**2*t_s) for n in range(n_terms))

def calcular_xc(mAE_val, mv, tf_s):
    mc = mAE_val + mv*tf_s
...
st.set_page_config(layout="wide",
                   page_title="Simulador Destilacion AE - Schinus Areira L.",
                   page_icon="🌿")
...
with st.sidebar:
    ...
    st.subheader("1. Condiciones de operacion")
    PeR   = st.number_input("Potencia resistencia PeR (W)", value=1000.0, step=100.0, format="%.1f")
    eta_c = st.slider("Fraccion calor perdido eta_c", 0.0, 0.5, 0.0, 0.01, format="%.2f")
    ...
    tf_min = st.slider("Tiempo de destilacion tf (min)", 10, 300, 88, 5)
    tf_s   = tf_min*60.0
    t_cal_s= t_cal*60.0
    t_total_min = tf_min + t_cal

    st.subheader("2. Material vegetal y difusion")
    VMV    = st.number_input("Volumen MV - VMV (cm3)", value=VMV_BASE, step=100.0, format="%.0f")
    C0     = st.number_input("Conc. inicial AE - C0 (g/cm3)", value=C0_BASE, step=0.001, format="%.5f")
    Deff   = st.number_input("Difusividad efectiva Deff (1/s)", value=Deff_BASE,
                              step=1e-5, format="%.5f",
                              help="Obtenida por ajuste de minimos cuadrados con datos experimentales")
    rho_AE = st.number_input("Densidad AE - rho (g/mL)", value=rho_AE_BASE, step=0.01, format="%.3f")

    st.subheader("3. Parametros economicos")
    ...
    CF       = st.number_input("Costos fijos CF ($/lote)", value=CF_BASE, step=500.0, format="%.0f",
                                help="8 h mano de obra x $1.815/h (SMVM, Res. 9/2025)")
...
try:
    ...
    t_cal_arr  = np.arange(60, t_cal_s+60, 60)
    t_dest_arr = np.arange(60, tf_s+60, 60)
    ...
    for t_i in t_cal_arr:
        Ce_i = calcular_Celect(0, t_i, PeR, Cv)
    ...
    # 6. RESULTADOS
    # ==============================================================================
    st.info(f"Tiempo caracteristico: tc = {fmt(tc_min,1)} min   |   "
            f"Caudal de vapor: mv = {fmt(mv*60,3)} g/min   |   "
            f"Masa inicial AE: MAE0 = {fmt(MAE0,2)} g   |   "
            f"Tiempo total: {fmt(t_total_min,0)} min")

    tab1, tab2 = st.tabs(["Resultados", "Grafico G(t)"])

    with tab1:
        st.subheader(f"Resultados a tf = {tf_min} min de destilacion (total {t_total_min:.0f} min)")
        ...
        st.markdown("---")
        st.markdown("##### Economico")
        e1, e2, e3, e4 = st.columns(4)
        e1.metric("Ingresos I(tf)",       f"$ {fmt(I_val,0)}")
        e2.metric("Costo electrico",      f"$ {fmt(Celect_val,0)}")
        ...
        a2.metric("Precio minimo AE (G=0)", f"$ {fmt(PrecioAE_min,0)} /mL")
        a3.metric("Tiempo total operacion", f"{fmt(t_total_min,0)} min")

        st.markdown("---")
        if t_eq:
            st.success(f"El proceso es rentable a partir de {fmt(t_eq,1)} min desde el encendido.")
        else:
            if G_val > 0:
                st.success("El proceso es rentable en todo el rango simulado.")
            else:
                st.warning("El proceso no es rentable. Revisa los parametros economicos.")

    with tab2:
        st.subheader("Analisis economico - Tiempo total desde encendido del equipo")
        fig, ax = plt.subplots(figsize=(11, 5))

        ax.plot(t_total_arr, I_arr,  color='steelblue', lw=2, label='Ingresos I(t)')
        ax.plot(t_total_arr, CT_arr, color='firebrick',  lw=2, label='Costos totales CO(t) + CF')
        ax.plot(t_total_arr, G_arr,  color='seagreen',   lw=2.5, label='Ganancia neta G(t)')
        ax.axhline(0, color='black', lw=0.8)

        ax.axvline(t_cal, color='orange', lw=1.5, ls='--',
                   label=f'Inicio destilacion (t = {t_cal:.0f} min)')
        ...
        ax.set_title('Ganancia neta G(t) en funcion del tiempo de operacion', fontsize=12)
    ...
    st.error(f"Error en el calculo: {ve}")
